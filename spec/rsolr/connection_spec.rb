...
describe RSolr::Connection do
  describe '#get' do
    context 'with valid basic auth credentials' do
      ...
      let(:client) { RSolr.connect(:url => 'http://user:pass@127.0.0.1:8983/solr') }
      ...
      specify { result.request[:uri].to_s.should eq('http://user:pass@127.0.0.1:8983/solr/select?wt=ruby&q=a') }
